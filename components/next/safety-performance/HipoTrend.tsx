    ...
    <svg viewBox={`0 0 ${w} ${h}`} className={className} aria-label="Tren HIPO">
      <defs>
        ...
        <clipPath id={`${uid}-clip`}>
          <motion.rect
            ...
            height={h}
            initial={{ width: 0 }}
            animate={{ width: play || visible ? fullW : 0 }}
            transition={
              play
                ? { duration: drawDur, ease: easeOut }
                : { duration: 0.01 }
            }
          />
        </clipPath>
      ...
      {play && (
        <motion.circle
          ...
          initial={{ opacity: 1, cx: cxKeys[0], cy: cyKeys[0] }}
          animate={{
            cx: cxKeys,
            cy: cyKeys,
            opacity: [1, 1, 0],
      ...
      <motion.line
        ...
        initial={{ opacity: 0 }}
        animate={{ opacity: play || visible ? 0.75 : 0 }}
        transition={play ? { duration: 0.35, delay: drawDur * 0.85 } : { duration: 0.2 }}
      />

      {HIPO.values.map((v, i) => {
        const isLast = i === last;
        const appearDelay = play ? (i / Math.max(1, last)) * drawDur * 0.85 : 0;
        return (
          <g key={HIPO.labels[i]}>
            {isLast && play && (
              <motion.circle
                ...
                initial={{ r: rLast, opacity: 0.55 }}
                animate={{ r: large ? 26 : 16, opacity: 0 }}
                transition={{ duration: 0.9, delay: drawDur * 0.85, ease: "easeOut" }}
              />
            )}
            <motion.circle
              cx={sx(i)}
              ...
              transition={
                play
                  ? { duration: 0.3, delay: appearDelay, ease: easeOut }
                  : { duration: 0.2 }
              }
            />
            <motion.text
              ...
              textAnchor="middle"
              initial={{ opacity: 0 }}
              animate={{ opacity: play || visible ? 1 : 0 }}
              transition={
                play
                  ? { duration: 0.25, delay: appearDelay + 0.08 }
                  : { duration: 0.2 }
              }
            >
              {fmt(v)}
